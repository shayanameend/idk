...
useMount(() => {
	...
	if (emblaNode) {
		viewportNode = emblaNode?.querySelector(".embla__viewport") as HTMLElement;
	}
...
>
  <div class="p-2 lg:p-16 row-span-3 col-span-3 grid place-items-center">
    <h3 id="heading" class="relative z-10 py-8 text-7xl md:text-[6.5rem]">
      {#each tagline.split(" ") as word}
        <span class="opacity-0">{word}</span>
        {#if word !== tagline.split(" ")[tagline.split(" ").length - 1]}
          <br />
        {/if}
      {/each}
